...
void getEstimatedAttitude_underwater();
  ...
  //gyro only: the delay to read 2 consecutive values can be reduced to only 0.65ms
    ...
    for (axis = 0; axis < 3; axis++) {
      ...
      //IFM
       ...
       {
         getEstimatedAttitude_underwater();
       }
    #endif
    #if GYRO
      Gyro_getADC();
...
//  #endif
  ...
  // Attitude of the estimated vector
  int32_t sqGX_sqGZ = sq(EstG32.V.X) + sq(EstG32.V.Y);
  invG = InvSqrt(sqGX_sqGZ + sq(EstG32.V.Z));
  att.angle[ROLL]  = _atan2(EstG32.V.X , EstG32.V.Z);
  att.angle[PITCH] = _atan2(-1*EstG32.V.Z , InvSqrt(sqGX_sqGZ)*sqGX_sqGZ);

  #if MAG
    att.heading = _atan2(
      -1*EstM32.V.X * EstG32.V.Z + EstM32.V.Z * EstG32.V.X,
      (EstM.V.Y * sqGX_sqGZ  - (EstM32.V.Z * EstG32.V.Z + EstM32.V.X * EstG32.V.X) * EstG.V.Y)*invG ); 
    att.heading += conf.mag_declination; // Set from GUI
    att.heading /= 10;
  #endif
