...
void ACC_getADC () {
  ...
  if(quadRunningMode == AIR_MODE) //AIR
  {
  ACC_ORIENTATION_AIR( ((rawADC[1]<<8) | rawADC[0])>>4 ,
                   ((rawADC[3]<<8) | rawADC[2])>>4 ,
                   ((rawADC[5]<<8) | rawADC[4])>>4 );
  }
  else
  {
   ACC_ORIENTATION_UNDERWATER( ((rawADC[1]<<8) | rawADC[0])>>4 ,
                   ((rawADC[3]<<8) | rawADC[2])>>4 ,
                   ((rawADC[5]<<8) | rawADC[4])>>4 );
  }
...
if(quadRunningMode == AIR_MODE) //AIR
{
  GYRO_ORIENTATION_AIR( ((rawADC[0]<<8) | rawADC[1])>>2 , // range: +/- 8192; +/- 2000 deg/sec
                    ((rawADC[2]<<8) | rawADC[3])>>2 ,
                    ((rawADC[4]<<8) | rawADC[5])>>2 );
}
else
{
  GYRO_ORIENTATION_UNDERWATER( ((rawADC[0]<<8) | rawADC[1])>>2 , // range: +/- 8192; +/- 2000 deg/sec
                    ((rawADC[2]<<8) | rawADC[3])>>2 ,
                    ((rawADC[4]<<8) | rawADC[5])>>2 );
}
  GYRO_Common();
...
//IFM
  if(quadRunningMode == AIR_MODE) //AIR
  {
    MAG_ORIENTATION_AIR( ((rawADC[0]<<8) | rawADC[1]) ,
                     ((rawADC[4]<<8) | rawADC[5]) ,
                     ((rawADC[2]<<8) | rawADC[3]) );
  }
  else
  {
    MAG_ORIENTATION_UNDERWATER( ((rawADC[0]<<8) | rawADC[1]) ,
                     ((rawADC[4]<<8) | rawADC[5]) ,
                     ((rawADC[2]<<8) | rawADC[3]) );
  }
  #endif
