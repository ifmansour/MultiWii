...
int temp = 0;
int16_t maxThrottle;
uint8_t pithRollRCRate;
uint8_t quadRunningMode;
...
void setup() {
  ...
  quadRunningMode = AIR_MODE;
  maxThrottle = MAXTHROTTLE_AIR;
  pithRollRCRate = PITCH_ROLL_RC_RATE_AIR;
...
void setMaxThrottle(int16_t maxThrottle_local)
{
  uint8_t i;
  maxThrottle = maxThrottle_local;
  for(i=0;i<11;i++) {
    ...
    //lookupThrottleRC[i] = conf.minthrottle + (int32_t)(MAXTHROTTLE_AIR-conf.minthrottle)* lookupThrottleRC[i]/1000;  // [0;1000] -> [conf.minthrottle;MAXTHROTTLE]
...
void setPithRollRCRate(int16_t pithRollRCRate_local)
{
   uint8_t i;
   pithRollRCRate = pithRollRCRate_local;
   conf.rcExpo8 = 0;
   conf.rcRate8 = pithRollRCRate;  
   for(i=0;i<5;i++) {
...
void loop () {
  ...
  if (currentTime > rcTime ) { // 50Hz
   ...
   if((rcData[AUX1PIN] <= 1200) &&(quadRunningMode == UNDERWATER_MODE))
   {
     quadRunningMode = AIR_MODE;
     setMaxThrottle(MAXTHROTTLE_AIR);
     setPithRollRCRate(PITCH_ROLL_RC_RATE_AIR);
   }
   else if((rcData[AUX1PIN] > 1200) &&(quadRunningMode == AIR_MODE))
   {
     quadRunningMode = UNDERWATER_MODE;
     setMaxThrottle(MAXTHROTTLE_UNDERWATER);
     setPithRollRCRate(PITCH_ROLL_RC_RATE_UNDERWATER);
   }
...
for(axis=0;axis<2;axis++) {          
    if((quadRunningMode == AIR_MODE) 
    #ifdef CLOSED_LOOP_UNDER_WATER
    || ((axis == 1) && (quadRunningMode == UNDERWATER_MODE))
    #endif
    )
    {
      ...
      if(quadRunningMode == AIR_MODE)
      {
          conf.pid[PITCH].P8 = 17;
          conf.pid[PITCH].D8 = 5;
      }
      else
      {
          conf.pid[PITCH].P8 = 10;
          conf.pid[PITCH].D8 = 3;
      }
    #endif  
     rc = rcCommand[axis]<<1;
...
//  if(quadRunningMode == UNDERWATER_MODE)  ///  <
//      axisPID[axis] =  PTerm + ITerm - DTerm;  
//   else 
//   {
//     // axisPID[axis] = rc;
